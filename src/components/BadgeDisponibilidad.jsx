 ...
 */
export default function BadgeDisponibilidad({ disponible, className = "", hacia = "abajo", compacto = false }) {
    ...
    <span ref={ref} className={`relative inline-block ${className}`}>
      <button
        ...
        className={[
          ...
          "inline-flex items-center gap-1.5 whitespace-nowrap rounded-control py-1 font-bold backdrop-blur-md transition-all duration-200",
          // En la tarjeta del catálogo (compacto) se achica un poco para que
          // "Entrega Inmediata" quepa completo en celulares angostos.
          compacto ? "px-1.5 sm:px-2.5 text-[11px] sm:text-xs" : "px-2.5 text-xs",
          disponible
      ...
      >
        <img
          src="/assets/icons/entrega-inmediata.png"
          alt=""
          className={compacto ? "hidden min-[400px]:block w-3.5 h-3.5 shrink-0" : "w-3.5 h-3.5 shrink-0"}
        />
      ...
      <span
        role="tooltip"
        className={[
          // En la tarjeta (compacto) la explicación no puede ser más ancha
          // que la tarjeta, si no se sale de la pantalla en la columna derecha.
          compacto
            ? "absolute z-20 left-0 w-[9.5rem] sm:w-max sm:max-w-[13rem] glass-dark text-ink text-xs sm:text-sm leading-snug rounded-control p-2.5"
